...
class AddPromotionItemActivity : AppCompatActivity() {
    ...
    private lateinit var promotionDiscount: String
    ...
    private lateinit var binding: ActivityAddPromotionItemBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAddPromotionItemBinding.inflate(layoutInflater)
        ...
        binding.addButton.setOnClickListener {
            ...
            promotionDiscount = binding.promotionDiscountPercentage.text.toString().trim()
            promotionStartDate = binding.startDate.text.toString().trim()
            promotionEndDate = binding.endDate.text.toString().trim()

            if (!(promotionName.isBlank() || promotionDescription.isBlank() || promotionTerms.isBlank() || promotionDiscount.isBlank() || promotionStartDate.isBlank() || promotionEndDate.isBlank())) {
                if (validateInputs()) {
                    uploadData()
                }
            } else {
    ...
    private fun validateInputs(): Boolean {
        val discount = promotionDiscount.toDoubleOrNull()
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    if (image != null) {
                        ...
                        val imageRef = storageRef.child("promotion_images/$newItemKey.jpg")
                        val uploadTask = imageRef.putFile(image!!)

                        uploadTask.addOnSuccessListener {
                            imageRef.downloadUrl.addOnSuccessListener { downloadUri ->
                                val promotionDiscountInt = promotionDiscount.toIntOrNull() ?: 0

                                val newItem = Promotion(
                                    promotionId = newItemKey ?: "",
                                    name = promotionName,
